...
- A live deploy can look "half updated" if the HTML is fresh but still points at an old `styles.css?v=` or `script.js?v=` URL. When that happened on `2026-05-13`, text changes appeared live while the mobile scaling fix stayed stale until the shared asset query strings were bumped across the HTML files and the bundle was rebuilt.
